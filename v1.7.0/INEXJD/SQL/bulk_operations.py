...
def bulk_insert(table_name, records):
    ...
    try:
        ...
        for record in records:
            # Check that all required table fields are present (Issue4: Less strict validation)
            for field in table_structure:
                if field not in record:
                    return f"Record {record} missing required field '{field}'"
    ...
    except Exception as e:  # Issue5: Exception handling
        return f"Bulk insert failed: {str(e)}"
...
def bulk_update(table_name, condition, new_data):
    ...
    """
    try:
        # Issue1: Add table existence check
        table_structure = getTableStructure(table_name)
        if isinstance(table_structure, str):
            return "Table not found"
...
def bulk_delete(table_name, condition):
    ...
    """
    try:
        # Issue2: Add table existence check
        table_structure = getTableStructure(table_name)
        if isinstance(table_structure, str):
            return "Table not found"
        ...
        return f"Bulk delete failed: {str(e)}"
